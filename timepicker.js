...
var content = $('<div class="timepicker">\
		<div v-show class="title">Выберите время </div><a href="javascript://" class="js-clear-input">[очистить]</a>\
			<div class="chose-all">\
				<div class="handle">\
				...
				<div class="text">\
					<div class="cell-4"><a class="js-hour-show" title="Выбрать часы"></a></div>\
					<div class="cell-2">:</div>\
					<div class="cell-4"><a class="js-minute-show" title="Выбрать минуты"></a></div>\
				</div>\
				<div class="handle">\
...
timepicker.bindEvent = function () {
	...
	// clear
	this.content.on('click','.js-clear-input',function() {
        thisTimePicker.inputTarget.val( "" );
        thisTimePicker.content.off().remove();
        thisTimePicker.hasBind = false;
	...
	// 选择分钟
		...
		thisTimePicker.title.text('Была выбрана минута..');
	...
	// 选择小时
		...
		thisTimePicker.choseAll.show();
		thisTimePicker.title.text('Был выбран час..');
	...
	thisTimePicker.hourShow.on('click',function() {
		...
		thisTimePicker.title.text('Выберите час');
	});
	...
	thisTimePicker.minuteShow.on('click',function() {
		...
		thisTimePicker.title.text('Выберите минуту');
	});
};
